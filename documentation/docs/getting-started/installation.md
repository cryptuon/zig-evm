...
The Rust crate links against `libzigevm` (`#[link(name = "zigevm")]`), so
you need to have built the shared library first (`zig build lib`) and
ensure it is discoverable on the linker path.
