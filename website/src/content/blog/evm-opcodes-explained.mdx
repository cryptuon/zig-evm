...
The `PUSH1` opcode reads the next byte from the bytecode stream and pushes it onto the stack. The `ADD` opcode pops two values, adds them, and pushes the result. You can try this yourself in the [Zig EVM Playground](/zig-evm/playground/).
...
You can paste this bytecode into the [Zig EVM Playground](/zig-evm/playground/) to step through it instruction by instruction.
...
For a deeper exploration of gas costs and optimization strategies, see our guide on [Understanding EVM Gas](/zig-evm/blog/understanding-evm-gas/).
...
| Tool | Type | Best For |
|------|------|----------|
| [Zig EVM Playground](/zig-evm/playground/) | Web-based editor + debugger | Writing raw bytecode, stack visualization, execution tracing |
...
The [Zig EVM Playground](/zig-evm/playground/) is unique in providing a Catppuccin-themed bytecode editor with real-time disassembly and stack visualization — ideal for learning how opcodes transform the stack step by step.
...
- You can experiment with raw bytecode in the [Zig EVM Playground](/zig-evm/playground/) or explore the full source on [GitHub](https://github.com/cryptuon/zig-evm).
